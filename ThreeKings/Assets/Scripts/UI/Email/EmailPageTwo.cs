...
public class EmailPageTwo : MonoBehaviour
{
    ...
    void InitNetData()
    {
        EmailDb.GetEmailSheet(3, DataManager.Instance.RoleID, (SheetData) => {
    ...
    void OnCreateItem(List<EmailSheetItem> sheetItems)
    {
        ...
        for (int i = 0; i < sheetItems.Count; i++)
        {
            ...
            emailSheet.InitDataTwo(this, sheetItems[m]);
        }
    ...
    void SystemDetail(int id)
    {
        EmailDb.GetEmailInfo(id, (infoData) => {
            if (infoData != null)
            {
                ...
                if (infoData.data.info.system != null)
                {
